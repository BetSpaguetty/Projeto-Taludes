...
class AmbienteController :
    ...
    def _initialization(self) :
        self.view.matrixViewer.setMatrixElevation(self.mapa.getElevationMatrix(), self.mapa.getXScaleVector(), self.mapa.getYScaleVector())
        ...
        self.dropBoxBackgroundColor.setCurrentIndex(0)
        self.spinboxScale.setValue(25)
    ...
    def renderMatrix(self, matrix) : 
        self.view.matrixViewer.renderMatrix(matrix)
    ...
    def functionDropBoxBackgroundColor(self, text) : 
        color = MATRIX_BACKGROUNDS[text]
        self.view.matrixViewer.setMatrixBackgroundColor(color)


    def functionSpinScale(self, scale): 
        self.view.matrixViewer.setScale(scale)
    ...
    def _associateGraphConfigs(self) :
        ...
        # COLOR MAP ----------------------------- >>>
        self.mapColorMaps = {}
        self.dropboxColorMaps = self.view.matrixViewer.addComboBoxBottom()
        ...
        # SHADERS ------------------------------- >>>
        self.dropBoxShaders = self.view.matrixViewer.addComboBoxBottom()
        for shader in SHADERS : 
            self.dropBoxShaders.addItem(shader)
        self.dropBoxShaders.currentIndexChanged.connect(self.functionDropBoxSahder)

        # BACKGROUND COLOR ---------------------- >>>
        self.dropBoxBackgroundColor= self.view.matrixViewer.addComboBoxBottom()
        for shader in MATRIX_BACKGROUNDS.keys() : 
            self.dropBoxBackgroundColor.addItem(shader)
        self.dropBoxBackgroundColor.currentTextChanged.connect(self.functionDropBoxBackgroundColor)

        # OPACIDADE ----------------------------- >>>
        self.sliderOpacity = self.view.matrixViewer.criar_slider_rotulado_bottom('Opacidade', 0, 100, 100)
        self.sliderOpacity.valueChanged.connect(self.functionSliderTransparency)

        # EIXOS --------------------------------- >>>
        self.buttonAxis = self.view.matrixViewer.addButtonBottom('Eixos', checkable=True)
        self.buttonAxis.clicked.connect(self.view.matrixViewer.setRenderAxis)

        # ESCALA -------------------------------- >>>
        self.spinboxScale = self.view.matrixViewer.addBottomSpinbox('Escala', 1, 100)
        self.spinboxScale.valueChanged.connect(self.functionSpinScale)
    ...
    def _associateFilters(self) : 
        # ELEVATION 
        self.buttonFilterElevation = self.view.matrixViewer.addButtonTop(GraphFilters.ELEVATION.value, checkable=True)
    ...
    def addFilter(self, filterName:str, filter:Filter) : 
        ...
        button = self.view.matrixViewer.addButtonTop(filterName, checkable=True)
    ...
    def functionFilter(self, filter:Filter, filterBar) :
        ...
        if len(list(filter.views.keys())) == 0 :
            self.removeToolbar()
        else : self.insertToolbar(filterBar)
