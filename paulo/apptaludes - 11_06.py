...
from PyQt5 import uic
from PyQt5.QtCore import Qt, QEvent
from PyQt5.QtWidgets import QAction, QToolBar, QStackedWidget, QWidget, QVBoxLayout, QDialog, QMessageBox, QApplication, QMainWindow, QGraphicsScene, QFileDialog, QPushButton, QGraphicsView, QLineEdit, QLabel, QGridLayout, QSizePolicy, QToolButton
from PyQt5.QtGui import QPixmap
from pyvistaqt import QtInteractor
...
class AppTaludes(QWidget):
    ...
    def runAnalysis(self):
        ...
        if self.number_of_adjusts >= 1:
            ...
            ax.plot_surface(x[2:x2-1, 2:y2-1], y[2:x2-1, 2:y2-1], z[2:x2-1, 2:y2-1], facecolors= plt.cm.rainbow_r(norm1(self.fos_array2[2:x2-1, 2:y2-1])), antialiased=False, shade=False) #[0:x2-1, 0:y2-1]
            ...
            ax.plot_surface(np.flipud(x.transpose()[2:x2-2, 2:y2-2]), np.flipud(y.transpose()[2:x2-2, 2:y2-2]), np.flipud(z.transpose()[2:x2-2, 2:y2-2]), facecolors= plt.cm.rainbow_r(norm1(np.flipud(self.fos_array2.transpose()[2:x2-2, 2:y2-2]))), antialiased=False, shade=False)
            m = mpl.cm.ScalarMappable(cmap= plt.cm.rainbow_r, norm= norm1)
            m.set_array(np.flipud(self.fos_array2.transpose()))
            plt.colorbar(m, ax= ax, location= "right")
    ...
    def runAnalysis2(self):
        ...
        if self.number_of_adjusts >= 1:
            ...
            ax.plot_surface(x[2:x2-2, 2:y2-2], y[2:x2-2, 2:y2-2], z.transpose()[2:x2-2, 2:y2-2], facecolors= plt.cm.rainbow_r(norm1(self.fos_array[2:x2-2, 2:y2-2])), antialiased=False, shade=False) #z.transpose()[2:x2-1, 2:y2-1], facecolors= plt.cm.rainbow_r(norm1(self.fos_array.transpose()[2:x2-1, 2:y2-1])), antialiased=False, shade=False)
            m = mpl.cm.ScalarMappable(cmap= plt.cm.rainbow_r, norm= norm1)
            m.set_array(self.fos_array.transpose())
            plt.colorbar(m, ax= ax, location= "right")
            ...
            plt.imshow(np.flipud(self.fos_array3.transpose()), cmap)
            im_fos = ax2.imshow(np.flipud(self.fos_array3.transpose()), cmap)
    ...
    def getfile(self):
        ...
        global fname
        fname, _ = QFileDialog.getOpenFileName(self, "Open File","Documents","Tif files(*.tif);;Tiff files(*.tiff)")

        # self.fname, _ = QFileDialog.getOpenFileName(self, "Open File","Documents","Tif files(*.tif);;Tiff files(*.tiff)")
    ...
    def elev_incl(self,arquivo):
        ...
        # self.horizontalLayout_3.addWidget(self.canvas)     #Trocado com o debaixo
        # self.horizontalLayout_browse.addWidget(self.toolb1)
        ...
        layout.addWidget(self.stacked_elevacao)

        # Add the PyVista widget to the stack
        #self.plotter = PyVistaWidget()
        #self.stacked_elevacao.addWidget(self.pv_widget)
        # print(self.exibe_elevacao)

        # self.Q = QWidget()
        # layout = QVBoxLayout(self)
        # self.plotter = QtInteractor(self.Q)
        ...
        #self.plotter.add_mesh(grid, scalars="elevation", cmap="terrain", show_scalar_bar=True, scalar_bar_args={'title': 'Elevação', 'vertical': True, 'position_x': 0.85, 'position_y': 0.25})

        pv.global_theme.window_size = [6000, 4000]
...
class Form2(QWidget):
    ...
    def __init__(self2,parent=None):
        ...
        pixmap = QPixmap("Projeto-Taludes\\paulo\\image.png")              #("Downloads\\mapa-rio-de-janeiro.jpg")
...
#class PyVistaWidget(QWidget):
    #def __init__(self, parent=None):
        ...
        #from pyvistaqt import QtInteractor
        #from PySide2.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QWidget, QStackedWidget)
        #layout = QVBoxLayout(self)
        #self.plotter = QtInteractor(QWidget())
