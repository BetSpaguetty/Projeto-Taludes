...
class AppTaludes(QWidget):
    ...
    def runAnalysis(self):
        ...
        print(imgx.size[1]-0.11)
        print(self.ui.imgsizey.text())

        img_arrayx = img_arrayx[0:int(self.ui.imgsizey.text()), 0:int(self.ui.imgsizex.text())]
    ...
    def getfile(self):
        ...
        g_max = np.zeros((img.size[0],img.size[1]));
        L = 25;
        Ld = np.sqrt(2)*L
        ...
        for i in range(1,img.size[1]-1): #Já testado o 199
            for j in range(1,img.size[0]-1):
                g_max[j,i] = max([abs(img_array[i+1,j]-img_array[i,j])/L,
                ...
                g_max[j,i] = np.arctan(g_max[j,i]) * 180/np.pi

        z_smoothed2 = sp.ndimage.gaussian_filter(g_max.transpose(), sigma)
        ...
        plt.imshow(g_max.transpose(), cmap='terrain')
        im = ax.imshow(g_max.transpose(), cmap='terrain')
        ...
        ax.set_yticks([0, 0.2*img.size[1], 0.4*img.size[1], 0.6*img.size[1], 0.8*img.size[1],img.size[1]], labels=[25*img.size[1], 20*img.size[1], 15*img.size[1], 10*img.size[1], 5*img.size[1], 0])
        ...
        self.ui.horizontalLayout_8.addWidget(self.toolb1)
        self.ui.horizontalLayout_3.addWidget(self.canvas)
        self.toolb2 = NavigationToolbar(self.canvas2, self)
        self.ui.horizontalLayout_8.addWidget(self.toolb2)
        # self.ui.horizontalLayout_4.addWidget(self.canvas2)

        self.tabInc = QWidget()
        self.tabInc_layout = QHBoxLayout()
    ...
    def figureAdjust(self):
        ...
        self.ui.horizontalLayout_8.removeWidget(self.toolb1)
        self.canvas.deleteLater()
        self.canvas2.deleteLater()
        self.toolb1.deleteLater()
        self.toolb2.deleteLater()
        plt.close('all')
        ...
        g_max2 = np.zeros((img2.size[0],img2.size[1]));
        L = 25;
        Ld = np.sqrt(2)*L
        ...
        for i in range(1,img2.size[1]-1): #Já testado o 199
            for j in range(1,img2.size[0]-1):
                g_max2[j,i] = max([abs(img_array2[i+1,j]-img_array2[i,j])/L,
                ...
                g_max2[j,i] = np.arctan(g_max2[j,i]) * 180/np.pi
        #

        # Creating second figure
        fig2, ax = plt.subplots(1,1,figsize=(12,10))
        plt.imshow(g_max2.transpose(), cmap='terrain')
        im = ax.imshow(g_max2.transpose(), cmap='terrain')
        ...
        ax.set_yticks([0, 0.2*img2.size[1], 0.4*img2.size[1], 0.6*img2.size[1], 0.8*img2.size[1],img2.size[1]], labels=[25*img2.size[1], 20*img2.size[1], 15*img2.size[1], 10*img2.size[1], 5*img2.size[1], 0])
        ...
        # self.inclina_tabs +=1
        self.tab_name = "Inclinação " + str(self.inclina_tabs)
        self.ui.tabs.insertTab(self.inclina_tabs,self.tabInc,self.tab_name)
    ...
    def clearCanvas(self):
        ...
        for i in reversed(range(self.ui.horizontalLayout_4.count())):
            self.ui.horizontalLayout_4.itemAt(i).widget().setParent(None)

        for i in reversed(range(self.ui.horizontalLayout_8.count())):
            self.ui.horizontalLayout_8.itemAt(i).widget().deleteLater()
